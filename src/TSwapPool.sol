...
contract TSwapPool is ERC20 {
    ...
    function deposit(
        ...
        // @audit-high  deadline is never used to check for deposits that have passed the deadline
        // IMPACT: the user can deposit even after the deadline has passed
        uint64 deadline
    )
        external
        ...
        returns (uint256 liquidityTokensToMint)
    {
        if (wethToDeposit < MINIMUM_WETH_LIQUIDITY) {
            // @audit-gas no return to emit min_weth since its a public func that anyone can lookup
            revert TSwapPool__WethDepositAmountTooLow(MINIMUM_WETH_LIQUIDITY, wethToDeposit);
        }
        if (totalLiquidityTokenSupply() > 0) {
            uint256 wethReserves = i_wethToken.balanceOf(address(this));
            // @audit-info line is not needed since the var is not being used
            ...
            // @audit-info would be better if it were above the interactions code to comply with CEI
        ...
        private
    {
        ...
        //@audit-low the last 2 params have been reversed
        ...
        returns (uint256 outputAmount)
    {
        ...
        //@audit-info MAGIC numbers being used here!
    ...
    {
        //@audit-high the Precision factor is incorrect!!! should be 1000
    ...
    function swapExactInput(
        ...
        /// @audit-info functions not used publically could be marked as internal to save gas
        public
        revertIfZero(inputAmount)
        revertIfDeadlinePassed(deadline)
    ...
    function swapExactOutput(
        ...
        IERC20 outputToken,
        uint256 outputAmount,
        // @audit-high there's no maxInputAmt here in the func args that should protect the user from the price slippage
        // !!
        uint64 deadline
    )
        public
        ...
        returns (uint256 inputAmount)
    {
        ...
        // @audit tehre shoudl be a check here to check if the inputAmt < maxInputAmt that the user is wiling to sepnd
        // to protect tehm from the price slippage
    ...
    function sellPoolTokens(uint256 poolTokenAmount) external returns (uint256 wethAmount) {
        // @audit the third param should be the weth token amt (i.e the  output token amt)
        // should have used the swapExactInput
    ...
    function _swap(IERC20 inputToken, uint256 inputAmount, IERC20 outputToken, uint256 outputAmount) private {
        ...
        //@audit-high remove this garbage; breaks the protocol!!!
        swap_count++;
        if (swap_count >= SWAP_COUNT_MAX) {
            swap_count = 0;
            // q what  is this value
            // @audit-info this 1_00... should be condensed to 1e18 and dont'use magic numbers
